...
const profile=fs.mkdtempSync(path.join(require('node:os').tmpdir(),'frameflow-layout-')),version=require('../package.json').version,dir=path.join(__dirname,'..','reports/V'+version);
const chrome=spawn(require('../scripts/browser-path.cjs')(),['--headless','--disable-background-networking','--no-first-run','--no-default-browser-check','--remote-debugging-port=0','--user-data-dir='+profile,'about:blank'],{stdio:['ignore','ignore','pipe']});
...
async function main(){
 ...
 server=http.createServer((req,res)=>{const name=req.url==='/'?'src/app.html':req.url.slice(1);if(!['src/app.html','frameflow-core.js','frameflow-app.js'].includes(name)){res.writeHead(404);res.end();return;}res.setHeader('Content-Type',name.endsWith('.js')?'application/javascript; charset=utf-8':'text/html; charset=utf-8');res.end(fs.readFileSync(path.join(__dirname,'..',name.startsWith('src/')?name:'src/'+name)));});await new Promise(r=>server.listen(0,'127.0.0.1',r));
