...
const C=require('../src/frameflow-core'),root=path.resolve(__dirname,'..'),file=path.join(root,'src/main.js'),dir=fs.mkdtempSync(path.join(os.tmpdir(),'frameflow-save-test-'));
let calls=0,cancel=false;const target=path.join(dir,'图片项目.json'),localRequire=createRequire(file);
const app={setName(){},commandLine:{getSwitchValue:()=>''},requestSingleInstanceLock:()=>true,on(){},whenReady:()=>({then(){}}),getPath:()=>dir};
