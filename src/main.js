...
const appRoot=__dirname,projectRoot=path.resolve(__dirname,'..');
const {Engine}=require(path.join(appRoot,'frameflow-engine'));
const C=require(path.join(appRoot,'frameflow-core'));
// Keep the historical user-data identity when changing the display name.
app.setName('FrameFlow');
...
const engine=new Engine(path.join(projectRoot,'runtime',process.platform));
...
function createWindow(){
  const page=path.join(appRoot,'app.html');
  window=new BrowserWindow({
    title:'课程剪辑 FrameFlow '+require('../package.json').version,width:1440,height:900,minWidth:1100,minHeight:700,
  ...
  if(app.commandLine.hasSwitch('acceptance-test'))window.webContents.once('did-finish-load',()=>require(path.join(projectRoot,'tests/native-acceptance.cjs'))(window,engine,app));
  if(app.commandLine.hasSwitch('timeline-test'))window.webContents.once('did-finish-load',()=>require(path.join(projectRoot,'tests/timeline-acceptance.cjs'))(window,app));
}
